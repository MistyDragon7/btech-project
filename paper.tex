...
\author{\IEEEauthorblockN{Arav Jain}
...
gap at $k{=}20$ (27.1\% vs.\ 17.0\%), at a modest 2.6\,pp Macro-F1
cost (0.858 vs.\ 0.884).
...
\begin{enumerate}
  ...
  \item A controlled ablation on 9,337 samples demonstrating that this
    ...
    attention maps (+10.1\,pp faithfulness gap at $k{=}20$).
...
rather than the trained model itself. The use of attention weights as
...
\begin{enumerate}
  ...
  \item Rank tokens by descending $\hat{\mathbf{a}}$; select top-$k$
    indices $\mathcal{I}_{\text{attn}}$.
  \item Mask $\mathcal{I}_{\text{attn}}$ by replacing with PAD;
    re-run inference to obtain $P_{\text{attn}}$.
  \item Repeat with $k$ uniformly random indices to obtain $P_{\text{rand}}$.
\end{enumerate}
...
than on random ones. We evaluate at $k \in \{5, 10, 20\}$ and report
means over 30 samples per family (240 total). $G > 0$ is a necessary---
not sufficient---condition for faithful explanation.
...
cosine annealing with $T_{\max}=100$ epochs, gradient clipping at 1.0,
and early stopping with patience 20 on validation Macro-F1. We use
3-fold stratified cross-validation with the same fold assignments
...
\midrule
GaussianNB     & 0.860\,$\pm$\,.001 & 0.782\,$\pm$\,.007 & 0.925\,$\pm$\,.007 \\
Decision Tree  & 0.922\,$\pm$\,.001 & 0.836\,$\pm$\,.004 & 0.910\,$\pm$\,.002 \\
LinearSVM      & 0.923\,$\pm$\,.001 & 0.844\,$\pm$\,.009 & 0.980\,$\pm$\,.005 \\
Markov Trans.  & 0.927\,$\pm$\,.004 & 0.858\,$\pm$\,.004 & 0.976\,$\pm$\,.003 \\
Plain Trans.   & 0.940\,$\pm$\,.003 & \underline{0.884\,$\pm$\,.009} & 0.981\,$\pm$\,.003 \\
BiLSTM         & \textbf{0.948\,$\pm$\,.004} & \textbf{0.894\,$\pm$\,.003} & \underline{0.992\,$\pm$\,.001} \\
Rand.\ Forest  & \underline{0.950\,$\pm$\,.003} & 0.893\,$\pm$\,.010 & \textbf{0.993\,$\pm$\,.001} \\
\bottomrule
\end{tabular}
\end{table}
...
$k \in \{5, 10, 20\}$. Both models pass the base faithfulness
criterion ($G > 0$ at all $k$). However, the Markov Transformer
produces markedly larger faithfulness gaps throughout, with the
advantage growing from 0.5\,pp at $k{=}5$ to 10.1\,pp at $k{=}20$.
...
\begin{table}[t]
\centering
\caption{Deletion test: mean probability drop $\Delta$ when masking top-$k$
  CLS-attention tokens vs.\ $k$ random tokens (240 samples, 8 families).}
\label{tab:faithfulness}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{llccc}
\toprule
\textbf{Model} & \textbf{Metric} & $k{=}5$ & $k{=}10$ & $k{=}20$ \\
\midrule
...
$k{=}20$. The Markov Transformer outperforms the plain variant in 6 of
8 families.

\begin{table}[t]
\centering
\caption{Per-family faithfulness gap $G$ at $k{=}20$.
...
SMS dispatch APIs emerges across test samples---APIs that form the
...
lower faithfulness gap---it has learned to classify from the overall
...
thus form high-weight transition edges---but these APIs are shared
...
27.1\% vs.\ 17.0\% at $k{=}20$. This improvement comes at a 2.6\,pp
...
structure: families with stereotyped API sequences benefit most, while
parasitic malware that blends into host-app traces benefits least.
