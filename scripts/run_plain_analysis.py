...
def load_plain_model():
    ...
    ckpt = torch.load(
        MODELS / "plain_transformer_best.pt", map_location="cpu", weights_only=False
    )
...
def run_analysis():
    ...
    padded = pad_with_truncation(
        encoded, max_len=cfg["max_seq_len"], truncation=cfg.get("truncation", "head")
    )
    ...
    criterion = nn.CrossEntropyLoss(weight=torch.from_numpy(class_w).to(device))
    ...
    report = classification_report(
        y_true, y_pred, target_names=family_names, output_dict=True, zero_division=0
    )
    ...
    for r in rows:
        print(
            f"  {r['family']:15s}  P={r['precision']:.3f}  R={r['recall']:.3f}"
