"""
Ablation (deprecated): plain vs gated.

NOTE:
- The codebase has been refactored to remove gated attention entirely.
- The main model is now a plain Transformer encoder with a learnable [CLS] token.
- This script is kept only as a historical artifact; it now runs the plain model
  and writes results to the same output paths to avoid breaking downstream
  aggregation scripts.
...
from src.preprocessing import APIVocabulary, load_dataset, pad_sequences
...
class Cfg:
    ...
    dropout = 0.1
    batch_size = 32
    epochs = 25  # shortened vs main experiment; gate effect visible earlier
    lr = 3e-4
    weight_decay = 1e-4
    patience = 7
    n_folds = 3
...
def main() -> None:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(padded, y), start=1):
        ...
        log.info(
            "Plain-transformer parameters: %d",
        ...
        train_loader = create_dataloader(
            X_train, y_train, batch_size=Cfg.batch_size, shuffle=True
        )
        test_loader = create_dataloader(
            X_test, y_test, batch_size=Cfg.batch_size, shuffle=False
        )
        ...
        log.info(
            "FOLD %d best: acc=%.4f f1=%.4f auc=%.4f",
            fold,
            best_metrics["accuracy"],
            best_metrics["f_score"],
            best_metrics["auc"],
        )
    ...
    out = REPO_ROOT / "results" / "ablation_summary.csv"
    with open(out, "w") as f:
        f.write(
            "model,accuracy_avg,accuracy_std,f_score_avg,f_score_std,auc_avg,auc_std\n"
        )
        f.write(
            f"PlainTransformer,"
    ...
    payload = {
        "model": "Transformer (CLS) — legacy ablation runner",
        "n_folds": Cfg.n_folds,
        "per_fold": fold_metrics,
        "aggregate": {k: {"mean": v[0], "std": v[1]} for k, v in agg.items()},
        "notes": "Gated attention has been removed from the codebase; this script no longer compares against a gated variant.",
    }
    with open(REPO_ROOT / "results" / "ablation_summary.json", "w") as f:
        json.dump(payload, f, indent=2, default=float)

    log.info("=" * 60)
    log.info("No-gate ablation aggregate:")
