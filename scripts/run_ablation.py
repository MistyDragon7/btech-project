"""
Ablation: train a plain (no-gate) transformer under the *identical* recipe
as GAME-Mal, on the *same stratified folds*. Writes results to
results/ablation_summary.csv so the paper can cite the real numbers.
...
def main() -> None:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(padded, y), start=1):
        ...
        model = GAMEMal(
            ...
            dropout=Cfg.dropout,
            use_gate=False,  # <-- ablation: no gating
        ).to(device)
        log.info("Plain-transformer parameters: %d", sum(p.numel() for p in model.parameters()))
    ...
    payload = {
        "model": "PlainTransformer (no gate)",
        "n_folds": Cfg.n_folds,
