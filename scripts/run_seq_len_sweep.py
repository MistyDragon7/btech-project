...
def main(quick: bool = False) -> None:
    ...
    y = np.array(labels)
    log.info(
        "Loaded %d samples across %d families (len>=30 filter)",
        len(sequences),
        len(family_names),
    )
    ...
    if Cfg.n_folds > 1:
        ...
        tr, te = train_test_split(
            np.arange(len(y)), test_size=0.3, stratify=y, random_state=Cfg.seed
        )
        splits = [(tr, te)]
    ...
    summary_path = out_dir / (
        "seq_len_sweep_summary_quick.csv" if quick else "seq_len_sweep_summary.csv"
    )
    ...
    best = summary_rows[0]
    log.info(
        "  max_seq_len=%d  truncation=%s  f1=%.4f (+/- %.4f)  acc=%.4f",
        best["max_seq_len"],
        best["truncation"],
        best["f_score_mean"],
        best["f_score_std"],
    ...
    with open(
        out_dir
        / (
            "seq_len_sweep_summary_quick.json"
            if quick
            else "seq_len_sweep_summary.json"
        ),
        "w",
    ) as f:
        json.dump(
