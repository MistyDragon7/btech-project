...
Writes: results/game_mal_final.json
        results/models/game_mal_best.pt
        results/models/vocab.pkl
...
log = logging.getLogger("game_mal_final")
...
def main() -> None:
    ...
    log.info("Final config: max_seq_len=%d  truncation=%s", max_len, trunc)
    if best_row:
        log.info("  (sweep mean F1=%.4f)", float(best_row["f_score_mean"]))
    ...
    out_path = REPO_ROOT / "results" / "game_mal_final.json"
