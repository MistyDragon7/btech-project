...
(Table~\ref{tab:dataset}). The base corpus contains
\textbf{9{,}337~samples} (after a default length $\geq 5$ filter
applied at load time); the transformer ablation and matched-gated
runs additionally apply a length $\geq 30$ filter, leaving
\textbf{8{,}085~samples}. The corpus spans families that together
cover four dominant malicious
...
prevents the majority class (Airpush at 63.0\%) from dominating
