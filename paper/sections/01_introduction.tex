...
\begin{itemize}
  ...
  We further \emph{validate faithfulness} via a deletion test:
  masking the top-$k$ highest-gate tokens drops the true-class
  probability significantly more than masking $k$ random tokens.

  \item An empirically-selected, reproducible \textbf{configuration
  policy}: a $\{256, 512, 768\} \times \{$head, tail$\}$
  sequence-length / truncation sweep selects $L_{\max}{=}512$ with
  head-truncation; the per-config 3-fold metrics are published so
  ...
  Gaussian Naïve Bayes), a faithful reproduction of D'Angelo et al.'s
  rule-voting classifier, a 2-layer BiLSTM as a non-attention
  sequence-model baseline, and GAME-Mal under identical stratified
  $k$-fold splits and seeds (both numpy and torch).
\end{itemize}
