...
To isolate the contribution of the G1 gating mechanism we train a
plain multi-head-attention transformer with the sigmoid gates
removed but all other architectural and training choices held
fixed (same $d_{\text{model}}$, heads, layers, training recipe, and
\emph{identical stratified folds}). Aggregate results appear in
Table~\ref{tab:ablation}.

\begin{table}[t]
\centering
\caption{Gated vs.\ non-gated transformer, averaged across
3 stratified folds. All other hyperparameters and folds identical.}
\label{tab:ablation}
\small
\begin{tabular}{lccc}
\toprule
...
\textbf{GAME-Mal} (gated)   & $0.940 \pm 0.002$ & $0.886 \pm 0.006$ & $0.984 \pm 0.002$ \\
\midrule
$\Delta$ (gate minus plain) & $-0.006$ & $-0.011$ & $-0.004$ \\
\bottomrule
\end{tabular}
\end{table}

The ablation is trained on a slightly shorter budget
(25~epochs, patience~7) than the main runs (40~epochs, patience~10),
chosen such that both models had stopped improving under their
respective early-stopping criteria at the checkpoints reported.

\paragraph{Reading the ablation honestly.}
The headline finding is the one we did \emph{not} expect: on
this 8{,}085-sample, 8-family corpus the G1 gate does not
improve the plain multi-head transformer in aggregate. The
non-gated baseline reaches a macro-F1 of $0.897 \pm 0.005$
against $0.886 \pm 0.006$ for the gated variant --- a $1.1$-pp
gap in favour of the plain model, larger than either run's
fold-to-fold standard deviation. Accuracy and AUROC follow the
same direction (plain ahead by $0.6$ and $0.4$ pp respectively).
